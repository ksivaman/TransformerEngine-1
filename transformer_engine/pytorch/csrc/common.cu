...
void dispatch_layernorm(
  ...
  nvte_layernorm_fwd(input_cu.data(), gamma_cu.data(), beta_cu.data(), scale_cu.data(), epsilon,
                     z_cu.data(), mu_cu.data(), rsigma_cu.data(), at::cuda::getCurrentCUDAStream(),
                     multiProcessorCount, workspace.data(), barrier.data(), amax_cu.data(),
                     scale_inv_cu.data());
...
void dispatch_cast_transpose_fusion(
  ...
  auto input_cu = makeTransformerEngineTensor(input, input_shape, input_type);
  auto output_cast_cu =
      makeTransformerEngineTensor(output_cast, output_cast_shape, output_cast_type);
  auto output_transpose_cu =
      makeTransformerEngineTensor(output_transpose, output_transpose_shape, output_transpose_type);
  auto scale_cu = makeTransformerEngineTensor(scale, scale_shape, scale_type);
  auto amax_cu = makeTransformerEngineTensor(amax, amax_shape, amax_type);
  auto scale_inv_cu = makeTransformerEngineTensor(scale_inv, scale_inv_shape, scale_inv_type);

  nvte_cast_transpose(input_cu.data(), scale_cu.data(), output_cast_cu.data(),
                      output_transpose_cu.data(), amax_cu.data(), scale_inv_cu.data(),
...
void dispatch_gelu(
  ...
  nvte_gelu(input_cu.data(), output_cu.data(), scale_cu.data(), amax_cu.data(), scale_inv_cu.data(),
            at::cuda::getCurrentCUDAStream());
}
...
void dispatch_bgrad_cast_transpose_fusion(
  ...
  auto scale_cu = makeTransformerEngineTensor(scale, scale_shape, scale_type);
  auto cast_output_cu =
      makeTransformerEngineTensor(cast_output, cast_output_shape, cast_output_type);
  auto transposed_output_cu = makeTransformerEngineTensor(
      transposed_output, transposed_output_shape, transposed_output_type);
  auto amax_cu = makeTransformerEngineTensor(amax, amax_shape, amax_type);
  ...
  nvte_cast_transpose_dbias(input_cu.data(), scale_cu.data(), cast_output_cu.data(),
                            transposed_output_cu.data(), amax_cu.data(), dbias_cu.data(),
                            scale_inv_cu.data(), workspace.data(),
                            at::cuda::getCurrentCUDAStream());
...
void dispatch_bgrad_dgelu_cast_transpose_fusion(
    ...
    const std::vector<size_t> &scale_inv_shape, const transformer_engine::DType scale_inv_type) {
  transformer_engine::TensorWrapper workspace;
  auto gelu_input_cu = makeTransformerEngineTensor(gelu_input, gelu_input_shape, gelu_input_type);
  auto input_cu = makeTransformerEngineTensor(input, input_shape, input_type);
  auto scale_cu = makeTransformerEngineTensor(scale, scale_shape, scale_type);
  auto cast_output_cu =
      makeTransformerEngineTensor(cast_output, cast_output_shape, cast_output_type);
...
void dispatch_multi_cast_transpose(
  ...
  std::vector<NVTETensor> input_list, scale_list, cast_output_list, transposed_output_list,
  ...
  for (size_t i = 0; i < input_dptr_list.size(); ++i) {
    ...
    scale_list.emplace_back(
        make_tensor(scale_dptr_list[i], scale_shape_list[i], scale_type_list[i]));
    cast_output_list.emplace_back(
    ...
    amax_list.emplace_back(make_tensor(amax_dptr_list[i], amax_shape_list[i], amax_type_list[i]));
    scale_inv_list.emplace_back(
        make_tensor(scale_inv_dptr_list[i], scale_inv_shape_list[i], scale_inv_type_list[i]));
  }
  ...
  // Launch TE kernel
  nvte_multi_cast_transpose(input_list.size(), input_list.data(), scale_list.data(),
                            cast_output_list.data(), transposed_output_list.data(),
                            amax_list.data(), scale_inv_list.data(),
                            at::cuda::getCurrentCUDAStream());
}
