...
void te_gemm(at::Tensor A, at::Tensor A_scale_inverse, transformer_engine::DType A_type,
  ...
  auto te_A_scale_inverse = makeTransformerEngineTensor(
      A_scale_inverse.data_ptr(), {1}, GetTransformerEngineDType(A_scale_inverse.scalar_type()));
  auto te_B = makeTransformerEngineTensor(
      B.data_ptr(), {static_cast<size_t>(B.size(0)), static_cast<size_t>(B.size(1))}, B_type);
  auto te_B_scale_inverse = makeTransformerEngineTensor(
      B_scale_inverse.data_ptr(), {1}, GetTransformerEngineDType(B_scale_inverse.scalar_type()));
  ...
  nvte_cublas_gemm(te_A.data(), te_A_scale_inverse.data(), te_B.data(), te_B_scale_inverse.data(),
                   te_D.data(), te_bias.data(), te_pre_gelu_out.data(), transa, transb, grad,
...
void fused_cast_transpose(at::Tensor input, at::Tensor scale, at::Tensor amax, at::Tensor scale_inv,
  ...
  dispatch_cast_transpose_fusion(input.data_ptr(), {M, N}, inp_type, scale.data_ptr(), {1},
                                 DType::kFloat32, input_cast.data_ptr(), {M, N}, otype,
                                 input_transpose.data_ptr(), {N, M}, otype, amax.data_ptr(), {1},
                                 DType::kFloat32, scale_inv.data_ptr(), {1}, DType::kFloat32);
}
...
std::vector<at::Tensor> fused_cast_transpose_bgrad(at::Tensor grad_output, at::Tensor scale,
  ...
  dispatch_bgrad_cast_transpose_fusion(
      grad_output.data_ptr(), {M, N}, grad_output_type, scale.data_ptr(), {1}, DType::kFloat32,
      grad_output_cast.data_ptr(), {M, N}, otype, grad_output_transpose.data_ptr(), {N, M}, otype,
      amax.data_ptr(), {1}, DType::kFloat32, grad_bias.data_ptr(), {N}, grad_output_type,
...
std::vector<at::Tensor> fused_cast_transpose_bgrad_dgelu(at::Tensor grad_output,
  ...
  dispatch_bgrad_dgelu_cast_transpose_fusion(
      grad_output.data_ptr(), {M, N}, grad_output_type, gelu_input.data_ptr(), {M, N},
      grad_output_type, scale.data_ptr(), {1}, DType::kFloat32, dgelu.data_ptr(), {M, N}, otype,
      dgelu_transpose.data_ptr(), {N, M}, otype, amax.data_ptr(), {1}, DType::kFloat32,
      grad_bias.data_ptr(), {N}, grad_output_type, scale_inv.data_ptr(), {1}, DType::kFloat32);
...
void fused_multi_cast_transpose(std::vector<at::Tensor> input_list,
  ...
  dispatch_multi_cast_transpose(
      input_dptr_list, input_shape_list, input_type_list, scale_dptr_list, scale_shape_list,
      scale_type_list, cast_output_dptr_list, cast_output_shape_list, cast_output_type_list,
      transposed_output_dptr_list, transposed_output_shape_list, transposed_output_type_list,
      amax_dptr_list, amax_shape_list, amax_type_list, scale_inv_dptr_list, scale_inv_shape_list,
      scale_inv_type_list);
}
...
at::Tensor fp8_transpose(at::Tensor input, transformer_engine::DType otype) {
  ...
  auto input_transpose = allocateTorchTensor(input.size(1), input.size(0), DType::kByte);
  dispatch_transpose(input.data_ptr(), {M, N}, otype, input_transpose.data_ptr(), {N, M}, otype);
...
at::Tensor fp8_gelu(at::Tensor input, at::Tensor scale, at::Tensor amax, at::Tensor scale_inv,
  ...
  auto output = allocateTorchTensor(input.size(0), input.size(1), DType::kByte);

  dispatch_gelu(input.data_ptr(), {M, N}, input_type, scale.data_ptr(), {1}, DType::kFloat32,
                output.data_ptr(), {M, N}, otype, amax.data_ptr(), {1}, DType::kFloat32,
                scale_inv.data_ptr(), {1}, DType::kFloat32);
...
std::vector<at::Tensor> layernorm_bwd(const at::Tensor &dz, const at::Tensor &x,
  ...
  nvte_layernorm_bwd(dz_cu.data(), x_cu.data(), mu_cu.data(), rsigma_cu.data(), gamma_cu.data(),
                     ...
                     at::cuda::getCurrentDeviceProperties()->multiProcessorCount, workspace.data(),
                     barrier.data());
...
std::vector<at::Tensor> layernorm_fwd_fp8(const at::Tensor &input, const at::Tensor &weight,
  ...
  dispatch_layernorm(input.data_ptr(), {N, H}, itype, weight.data_ptr(), {H}, itype,
                     bias.data_ptr(), {H}, itype, scale.data_ptr(), {1}, DType::kFloat32, eps,
                     ln_out.data_ptr(), {N, H}, otype, mu.data_ptr(), {N}, DType::kFloat32,
                     rsigma.data_ptr(), {N}, DType::kFloat32, amax.data_ptr(), {1}, DType::kFloat32,
                     scale_inv.data_ptr(), {1}, DType::kFloat32,
...
std::vector<at::Tensor> layernorm_fwd(const at::Tensor &input, const at::Tensor &weight,
  ...
  dispatch_layernorm(
      input.data_ptr(), {N, H}, itype, weight.data_ptr(), {H}, itype, bias.data_ptr(), {H}, itype,
      nullptr, {1}, DType::kFloat32, eps, ln_out.data_ptr(), {N, H}, itype, mu.data_ptr(), {N},
      DType::kFloat32, rsigma.data_ptr(), {N}, DType::kFloat32, nullptr, {1}, DType::kFloat32,
      nullptr, {1}, DType::kFloat32, at::cuda::getCurrentDeviceProperties()->multiProcessorCount);
...
at::Tensor cast_to_fp8(const at::Tensor &input, const at::Tensor &scale, at::Tensor amax,
  ...
  nvte_fp8_quantize(input_cu.data(), scale_cu.data(), output_cu.data(), amax_cu.data(),
                    scale_inv_cu.data(), at::cuda::getCurrentCUDAStream());
