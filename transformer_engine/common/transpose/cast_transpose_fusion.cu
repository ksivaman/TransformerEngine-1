...
template <bool full_tile, int nvec_in, int nvec_out, typename IVec, typename OVec, typename CVec,
          typename CType>
inline __device__ void cast_and_transpose_regs_partial_dbias(
    const IVec (&in)[nvec_out], OVec (&out_trans)[nvec_in],
    CVec &out_dbias,  // NOLINT(*)
    typename OVec::type *output_cast_tile, const size_t current_place, const size_t stride,
    CType &max,  // NOLINT(*)
    const CType scale, const int dbias_shfl_src_lane, const bool valid_store) {
...
#pragma unroll
    ...
    cast_and_transpose_regs_partial_dbias<true>(
        in[current_in ^ 1], out_trans, partial_dbias, my_output_c_tile, current_place, stride, max,
        scale, (my_id_in_warp + i + warp_id_in_tile * n_iterations) % THREADS_PER_WARP, true);
...
template <int nvec_in, int nvec_out, typename Param>
__global__ void __launch_bounds__(cast_transpose_num_threads)
    cast_transpose_dbias_kernel_notaligned(const Param param, const size_t row_length,
...
#pragma unroll
    ...
    cast_and_transpose_regs_partial_dbias<false>(
        in[current_in ^ 1], out_trans, partial_dbias, my_output_c_tile, current_place, stride, max,
        scale, (my_id_in_warp + i + warp_id_in_tile * n_iterations) % THREADS_PER_WARP,
        valid_store);
...
void reduce_dbias(const Tensor &workspace, Tensor *dbias, const size_t row_length,
  ...
  reduce_dbias_kernel<reduce_dbias_nvec, fp32, InputType>
      <<<reduce_dbias_num_blocks, reduce_dbias_num_threads, 0, stream>>>(
          ...
          reinterpret_cast<const fp32 *>(workspace.dptr), reduce_dbias_row_length,
          reduce_dbias_num_rows);
}

void cast_transpose_dbias(const Tensor &input, const Tensor &scale, Tensor *cast_output,
                          Tensor *transposed_output, Tensor *amax, Tensor *dbias, Tensor *scale_inv,
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          cast_output->dtype, OutputType, constexpr int itype_size = sizeof(InputType);
          ...
          using Param = CTDBiasParam<InputType, OutputType, ComputeType>; Param param;
          ...
          reduce_dbias<InputType>(*workspace, dbias, row_length, num_rows, nvec_out,
                                  stream););  // NOLINT(*)
  );                                          // NOLINT(*)
}
...
template <int nvec_in, int nvec_out, typename Param>
__global__ void __launch_bounds__(cast_transpose_num_threads)
    cast_transpose_dbias_dgelu_kernel(const Param param, const size_t row_length,
                                      const size_t num_rows, const size_t num_tiles) {
  using IType = typename Param::InputType;
  ...
  const IType *const my_input_tile =
      param.input + (tile_id_x * nvec_in + tile_id_y * row_length * nvec_out) * THREADS_PER_WARP;
  const IType2 *const my_gelu_input_tile =
      param.gelu_input +
...
#pragma unroll
    ...
    OVec out_trans[nvec_in];  // NOLINT(*)
    cast_and_transpose_regs_partial_dbias<true>(
        after_dgelu, out_trans, partial_dbias, my_output_c_tile, current_place, stride, max, scale,
...
template <int nvec_in, int nvec_out, typename Param>
__global__ void __launch_bounds__(cast_transpose_num_threads)
    cast_transpose_dbias_dgelu_kernel_notaligned(const Param param, const size_t row_length,
                                                 const size_t num_rows, const size_t num_tiles) {
  using IType = typename Param::InputType;
  using IType2 = typename Param::InputType2;
  using OType = typename Param::OutputType;
...
void cast_transpose_dbias_dgelu(const Tensor &input, const Tensor &gelu_input, const Tensor &scale,
                                Tensor *cast_output, Tensor *transposed_output, Tensor *amax,
                                Tensor *dbias, Tensor *scale_inv, Tensor *workspace,
                                cudaStream_t stream) {
  NVTE_CHECK(input.shape.size() == 2, "Input must have 2 dimensions.");
  NVTE_CHECK(cast_output->shape.size() == 2, "C output must have 2 dimensions.");
  NVTE_CHECK(transposed_output->shape.size() == 2, "T output must have 2 dimensions.");
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      input.dtype, InputType,
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          cast_output->dtype, OutputType, using InputType2 = InputType;
          /* dgelu fusion kernel uses more registers */
          constexpr int desired_load_size_dgelu = 4;
          constexpr int desired_store_size_dgelu = 4; constexpr int itype_size = sizeof(InputType);
          constexpr int otype_size = sizeof(OutputType);
          ...
          Param param; param.input = reinterpret_cast<const InputType *>(input.dptr);
          ...
          param.amax = reinterpret_cast<ComputeType *>(amax->dptr);
          param.scale_inv = reinterpret_cast<ComputeType *>(scale_inv->dptr);
          param.workspace = reinterpret_cast<ComputeType *>(workspace->dptr); if (full_tile) {
...
void nvte_cast_transpose_dbias(const NVTETensor input, const NVTETensor scale,
  ...
  cast_transpose_dbias(
      *reinterpret_cast<const Tensor *>(input), *reinterpret_cast<const Tensor *>(scale),
      reinterpret_cast<Tensor *>(cast_output), reinterpret_cast<Tensor *>(transposed_output),
      reinterpret_cast<Tensor *>(amax), reinterpret_cast<Tensor *>(dbias),
      reinterpret_cast<Tensor *>(scale_inv), reinterpret_cast<Tensor *>(workspace), stream);
}
...
void nvte_cast_transpose_dbias_dgelu(const NVTETensor input, const NVTETensor gelu_input,
                                     ...
                                     cudaStream_t stream) {
  using namespace transformer_engine;
  cast_transpose_dbias_dgelu(
      *reinterpret_cast<const Tensor *>(input), *reinterpret_cast<const Tensor *>(gelu_input),
      *reinterpret_cast<const Tensor *>(scale), reinterpret_cast<Tensor *>(cast_output),
      reinterpret_cast<Tensor *>(transposed_output), reinterpret_cast<Tensor *>(amax),
      reinterpret_cast<Tensor *>(dbias), reinterpret_cast<Tensor *>(scale_inv),
