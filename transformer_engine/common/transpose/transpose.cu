...
inline __device__ void transpose_regs(const IVec (&in)[nvec_out], OVec (&out_trans)[nvec_in]) {
...
__global__ void __launch_bounds__(cast_transpose_num_threads)
    transpose_kernel(const IType *const input, OType *const output_t, const size_t row_length,
...
template <int nvec_in, int nvec_out, typename CType, typename IType, typename OType>
__global__ void __launch_bounds__(cast_transpose_num_threads)
    transpose_kernel_notaligned(const IType *const input, OType *const output_t,
...
void transpose(const Tensor &input, Tensor *transposed_output, cudaStream_t stream) {
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
      input.dtype, Type, constexpr int type_size = sizeof(Type);
      ...
      if (full_tile) {
        ...
        transpose_kernel<nvec_in, nvec_out, fp32, Type, Type>
            <<<n_blocks, cast_transpose_num_threads,
               ...
               stream>>>(reinterpret_cast<const Type *>(input.dptr),
                         reinterpret_cast<Type *>(transposed_output->dptr), row_length, num_rows,
                         n_tiles);
      } else {
...
void nvte_transpose(const NVTETensor input, NVTETensor transposed_output, cudaStream_t stream) {
  ...
  transpose(*reinterpret_cast<const Tensor *>(input), reinterpret_cast<Tensor *>(transposed_output),
            stream);
}
