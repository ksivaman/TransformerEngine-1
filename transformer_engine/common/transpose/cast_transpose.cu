...
__global__ void __launch_bounds__(cast_transpose_num_threads)
    cast_transpose_kernel(const IType *const input, OType *const output_c, OType *const output_t,
                          const CType *const scale_ptr, CType *const amax, CType *const scale_inv,
                          const size_t row_length, const size_t num_rows, const size_t num_tiles) {
...
#pragma unroll
    ...
    cast_and_transpose_regs<true>(in[current_in ^ 1], out_trans, my_output_c_tile, current_place,
                                  stride, max, scale, true);
#pragma unroll
    for (unsigned int j = 0; j < nvec_in; ++j) {
...
template <int nvec_in, int nvec_out, typename CType, typename IType, typename OType>
__global__ void __launch_bounds__(cast_transpose_num_threads)
    cast_transpose_kernel_notaligned(const IType *const input, OType *const output_c,
...
#pragma unroll
    ...
    cast_and_transpose_regs<false>(in[current_in ^ 1], out_trans, my_output_c_tile, current_place,
                                   stride, max, scale, valid_store);
#pragma unroll
    for (unsigned int j = 0; j < nvec_in; ++j) {
      out_space[i][j].data.vec = out_trans[j].data.vec;
    }
...
void cast_transpose(const Tensor &input, const Tensor &scale, Tensor *cast_output,
                    Tensor *transposed_output, Tensor *amax, Tensor *scale_inv,
                    cudaStream_t stream) {
          ...
          if (full_tile) {
            ...
            cast_transpose_kernel<nvec_in, nvec_out, fp32, InputType, OutputType>
                <<<n_blocks, cast_transpose_num_threads,
                   ...
                   stream>>>(reinterpret_cast<const InputType *>(input.dptr),
                             ...
                             reinterpret_cast<fp32 *>(scale_inv->dptr), row_length, num_rows,
                             n_tiles);
          } else {
            cudaFuncSetAttribute(
...
void nvte_cast_transpose(const NVTETensor input, const NVTETensor scale, NVTETensor cast_output,
  ...
  cast_transpose(*reinterpret_cast<const Tensor *>(input), *reinterpret_cast<const Tensor *>(scale),
