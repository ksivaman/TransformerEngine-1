...
layer_norm::FwdFunction &get_fwd_launcher(DType wtype, DType itype, DType otype, DType ctype,
  ...
  if (batch_size % 4 == 0 && layer_norm::FWD_TUNED_FUNCS.count(tuned_key) > 0) {
...
layer_norm::BwdFunction &get_bwd_launcher(DType wtype, DType itype, DType otype, DType ctype,
  ...
  // Look for tuned kernel
  auto tuned_key = layer_norm::get_key(wtype, itype, otype, ctype, hidden_size);
  if (batch_size % 4 == 0 && layer_norm::BWD_TUNED_FUNCS.count(tuned_key) > 0) {
...
void layernorm_fwd(const Tensor &x,      // BxSxhidden_size
                   ...
                   const Tensor &scale, const float epsilon, Tensor *z, Tensor *mu, Tensor *rsigma,
                   cudaStream_t stream, const int multiprocessorCount, Tensor *workspace,
                   Tensor *barrier, Tensor *amax, Tensor *scale_inv) {
  auto itype = x.dtype;
  auto wtype = gamma.dtype;
  auto otype = z->dtype;
  bool fp8_out = otype == DType::kFloat8E4M3 || otype == DType::kFloat8E5M2;
  ...
  auto launcher = layer_norm::get_fwd_launcher(wtype, itype, otype, ctype, hidden_size, rows);
  ...
  // Clear buffers
  if (params.fp8_out) {
    cudaMemsetAsync(params.amax, 0, layer_norm::product(amax->shape) * typeToSize(amax->dtype),
                    stream);
  }
  if (launch_params.barrier_size > 0) {
    cudaMemsetAsync(params.barrier, 0,
                    layer_norm::product(barrier->shape) * typeToSize(barrier->dtype), stream);
  }
...
void layernorm_bwd(const Tensor &dz, const Tensor &x, const Tensor &mu, const Tensor &rsigma,
  ...
  auto launcher = layer_norm::get_bwd_launcher(wtype, itype, otype, ctype, hidden_size, rows);
  ...
  if (dgamma_part->dptr == nullptr) {
    ...
    dgamma_part->shape = {static_cast<uint64_t>(launch_params.params.ctas_per_col), hidden_size};

    dbeta_part->dtype = ctype;
    dbeta_part->shape = {static_cast<uint64_t>(launch_params.params.ctas_per_col), hidden_size};
...
void nvte_layernorm_fwd(const NVTETensor x,      // BxSxhidden_size
                        ...
                        NVTETensor barrier, NVTETensor amax, NVTETensor scale_inv) {
  using namespace transformer_engine;
  layernorm_fwd(*reinterpret_cast<const Tensor *>(x), *reinterpret_cast<const Tensor *>(gamma),
                *reinterpret_cast<const Tensor *>(beta), *reinterpret_cast<const Tensor *>(scale),
                epsilon, reinterpret_cast<Tensor *>(z), reinterpret_cast<Tensor *>(mu),
                ...
                reinterpret_cast<Tensor *>(amax), reinterpret_cast<Tensor *>(scale_inv));
}
