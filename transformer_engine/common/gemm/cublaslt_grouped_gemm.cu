...
// (cuBLAS 13.2 was mostly functional but contained a bug for wgrad when a group had k=0,
// where the weight gradient would be uninitialized random data instead of zeros.)
...
inline size_t validate_grouped_gemm_inputs(
    ...
    if (tensor->has_data() || tensor->has_columnwise_data()) {
      if (!has_empty_logical_shape) {
        NVTE_CHECK(is_supported_input_dtype(tensor->dtype()),
                   "Grouped GEMM inputs must be FP8, BF16, FP16, or FP4, got ",
                   transformer_engine::to_string(tensor->dtype()), ".");
      }
    } else {
      NVTE_CHECK(has_empty_logical_shape,
  ...
  if (ref != nullptr) {
    ...
    for (const auto *tensor : inputs) {
      ...
      NVTE_CHECK(
          transformer_engine::is_fp8_block_scaling(tensor->scaling_mode) == ref_is_fp8_block,
...
inline GroupedOperandSelection select_grouped_operand(const transformer_engine::GroupedTensor *t,
  ...
  if (!has_row && !has_col) {
    ...
    sel.is_empty_logical_shape = has_empty_logical_shape;
    return sel;
  }
...
inline void execute_grouped_gemm(const GroupedGemmSetupWorkspace &setup_workspace,
  ...
  if (config.avg_m == 0 || config.avg_n == 0 || config.avg_k == 0) {
    return;
  }

  const int math_sm_count = config.math_sm_count;
...
__global__ void setup_grouped_gemm_kernel(
  ...
  // A/B offsets are in elements of logical datatype, so convert using bit-width
  // to correctly handle packed formats (e.g., NVFP4 / float4_e2m1fn_x2).
  A_ptrs[idx] =
...
void nvte_grouped_gemm(const NVTEGroupedTensor A, int transa, const NVTEGroupedTensor B, int transb,
  ...
  // Grouped GEMM requires Hopper (SM90) with cuBLAS 13.4+ or Blackwell (SM100+) with cuBLAS 13.3+.
  ...
  gemm_config.math_sm_count = config_.sm_count;
