    ...
    def test_grouped_mlp_srelu(
        ...
        *,
        ...
        for group_idx in range(group_size):
            x = xs[group_idx]
            x = torch.nn.functional.linear(x, fc1_ws_ref[group_idx], bias=fc1_bs_ref[group_idx])
            # SReLU(x) = max(x, 0) ** 2 (matches tex.srelu / cuDNN srelu kernel)
        ...
        if (
            ...
            fwd_cls = te_ops.fused.ForwardGroupedMLP_CuTeGEMM_MXFP8
            bwd_cls = te_ops.fused.BackwardGroupedMLP_CuTeGEMMD_MXFP8
            if fwd_cls.is_supported() and fwd_cls.is_activation_supported(scaled_act):
                forward_ops = module._module_groups[0]._forward_ops
                assert len(forward_ops) == 1
                assert isinstance(forward_ops[0][0], fwd_cls)
            if bwd_cls.is_supported() and bwd_cls.is_activation_supported(scaled_act):
                backward_ops = module._module_groups[0]._backward_ops
                assert len(backward_ops) == 1
                assert isinstance(backward_ops[0][0], bwd_cls)
