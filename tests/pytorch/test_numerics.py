...
def _make_mxfp8_quantizer(*, is_a: bool, transposed: bool) -> MXFP8Quantizer:
    if is_a:
        rowwise = transposed
...
def test_grouped_gemm_grouped_tensor(case, layout, accumulate) -> None:
    ...
    if case == "discrete_out" and (layout != "NT"):
        pytest.skip("discrete_out only covers NT")
    if layout == "NT" and case == "discrete_in":
        pytest.skip("NT is not supported for discrete_in.")
    ...
    if layout == "TN":
        ...
        grouped_out = (
            _make_grouped_tensor_from_splits(m_sizes, n, device, dtype)
        ...
        grouped_B = _make_grouped_tensor_from_splits(m_sizes, n, device, dtype)  # grad_output
        grouped_out = (
            _make_grouped_tensor_uniform(z, n, k, device, dtype)
            if case != "discrete_out"
            else grouped_out
        )  # wgrad
...
@pytest.mark.parametrize("layout", ["TN", "NN", "NT"])
def test_grouped_gemm_grouped_tensor_mxfp8(layout: str) -> None:
    torch.manual_seed(0)
    z = 3
    m_sizes = [512, 512, 512]
    n, k = 512, 512
